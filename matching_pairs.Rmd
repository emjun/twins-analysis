...
```{r}
#procedure proposal
# Question: What does "procedure proposal" mean?
colnames(twins2013)
...
# Question: Do we assume that matching on the above identifiers would lead to approx half of the obs because matching would lead to twins (2 entries/rows)?
...
# Question: Where does each number come from?
87363/110731
# Question: Where does each number come from? Why is the denominator different from the above line of code?
87363/123723

```

# What is the below chunk of code? It looks like an older/newer version. Can you integrate with thee above code somehow so we have only one version of the code? Don't want to accidentally edit/update the wrong version!
```{r}
library(tidyverse)
...
Output the matched CSV
```{r}
# Question: Is this the right one to output and analyze?
write.csv(twins_2013_2, './data/twins2013_matched.csv')
```